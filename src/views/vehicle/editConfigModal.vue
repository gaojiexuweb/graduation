<template>
<Modal :value="open" title="编辑配置" :loading="loading" @on-cancel="cancel">
    <footer slot="footer">
    ...
    <Form ref="formInline" :model="sendObj" :label-width="100">
        <FormItem prop="vehicleType" label="车辆型号" required>
            <Input v-model="sendObj.vehicleType" type="text" disabled></Input>
        </FormItem>
...
export default {
    ...
    methods: {
        commit(sendObj) {
            if (!this.sendObj.vehicleType) {
                this.$popError('请输入车辆型号');
                this.changeLoading()
                return
            } else if (!this.sendObj.vehicleNumber) {
                this.$popError('请输入车辆编号');
                this.changeLoading()
                return
                ...
                this.changeLoading()
                return
            } else if (!this.sendObj.color) {
                this.$popError('请输入颜色');
                this.changeLoading()
                return
            } else if (!this.sendObj.length) {
                this.$popError('请输入长度');
                this.changeLoading()
                return
            } else if (!this.sendObj.width) {
                this.$popError('请输入宽度');
                this.changeLoading()
                return
            } else if (!this.sendObj.height) {
                this.$popError('请输入高度');
                this.changeLoading()
                return
            } else {
